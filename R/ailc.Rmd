...
glm_fit_2 <- glm(Number_Of_Deaths ~
    ...
    # Issue_Year + 
    # Calendar_Year - 1,
      splines::ns(Attained_Age, knots=c(45, 60, 85))*splines::ns(Face_Amount, knots=c(37.5e3, 200e3, 500e3))
    + Issue_Year
    + splines::ns(Duration, knots=c(20, 30))
    + splines::ns(Issue_Age, knots=c(70)) +
    + Calendar_Year,
  data=model_data.train,
  family = poisson()
)
...
xgboost::xgb.plot.shap(
  data=model_data.train.xgb, model=xgb_model, top_n = 3
)
...
model_data.test$Predicted_Deaths <- predict(
  glm_fit_2$relaxed,
  newx=model.matrix(model_formula, model_data.test),
...
X_df <- cbind(
  model.matrix(model_formula, model_data.train) %>% as_tibble())

X_df.keep <- X_df %>%
  select(any_of(keep_model_terms))

list(
  "kappa (X_df.keep)" = kappa(X_df.keep %>% as.matrix()),
  "ratio (X_df / X_df.keep)" = kappa(X_df %>% as.matrix()) / kappa(X_df.keep %>% as.matrix())
)
...
stan_formula <- as.formula(
  paste0("Number_Of_Deaths ~ ", paste(paste0("`", colnames(X_df.keep), "`"), collapse = " + ")))

X_df.stan <- cbind(
  X_df.keep,
  model_data.train %>% select(Number_Of_Deaths)
)
...
X_df <- model.matrix(model_formula, model_data.test) %>%
      as.tibble()
