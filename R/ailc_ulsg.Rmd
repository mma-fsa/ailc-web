...
model_formula <- Number_Of_Deaths ~ 
    splines::ns(Issue_Age, knots=c(50, 75)) * splines::ns(Face_Amount, knots=c(17.5e3, 75e3)) +
    splines::ns(Attained_Age, knots=c(65)) +
    splines::ns(Issue_Age, knots=c()):splines::ns(Issue_Year, knots=c(2002)) +
    Preferred_Class * Gender - 1
...
sum(model_data.train$Number_Of_Deaths) / sum(predict(xgb_model, xgb_data))
...
xgboost::xgb.plot.shap(
  data=model_data.train.xgb, model=xgb_model, top_n = 5
)
...
X_df <- cbind(
  model.matrix(model_formula, model_data.train) %>% as.tibble())

X_df.keep <- X_df %>%
  select(any_of(keep_model_terms))

kappa(X_df %>% as.matrix())

```

```{r}

kappa(X_df.keep %>% as.matrix())
...
stan_formula <- as.formula(
  paste0("Number_Of_Deaths ~ ", paste(paste0("`", colnames(X_df.keep), "`"), collapse = " + "), " - 1"))
...
model_data.test["Predicted_Deaths"] <- predict(
  glm_fit_2,
  newx = model_data.test %>% as.matrix(),
  newoffset = log(model_data.test$Expected_Deaths),
  type = "response"
)
