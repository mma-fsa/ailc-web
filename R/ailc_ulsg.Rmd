...
X_df <- cbind(model.matrix(model_formula, model_data.train) %>%
      as.tibble())
...
# )
# 
keep_idx <- vapply(keep_model_terms,
                   function(x) { which(x == colnames(X_mat))}, 0)

stan_formula <- as.formula(
  paste0("Number_Of_Deaths ~ ", paste(paste0("`", colnames(X_mat.keep), "`"), collapse = " + "), " - 1"))


glm_fit_final.stan <- stan_glm(
  stan_formula,
  data = cbind(
    model.matrix(model_formula, model_data.train) %>%
      as.tibble(),
    tibble(
      Number_Of_Deaths = model_data.train$Number_Of_Deaths
    )),
  offset=log(model_data.train$Expected_Deaths),
  family=poisson(),
  iter=2000,
